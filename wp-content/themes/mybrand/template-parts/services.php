<?php
$services = [
    [
        'slug'  => 'supported-living',
        'title' => __( 'Supported Living', 'mybrand' ),
        'desc'  => __( 'Enabling adults with complex needs to live independently in their own homes with tailored 24/7 support packages.', 'mybrand' ),
        'icon'  => '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.75" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 12l8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25"/></svg>',
    ],
    [
        'slug'  => 'learning-disabilities',
        'title' => __( 'Learning Disabilities', 'mybrand' ),
        'desc'  => __( 'Specialist care and support for people with mild to profound learning disabilities, focusing on independence and quality of life.', 'mybrand' ),
        'icon'  => '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.75" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M4.26 10.147a60.436 60.436 0 00-.491 6.347A48.627 48.627 0 0112 20.904a48.627 48.627 0 018.232-4.41 60.46 60.46 0 00-.491-6.347m-15.482 0a50.57 50.57 0 00-2.658-.813A59.905 59.905 0 0112 3.493a59.902 59.902 0 0110.399 5.84c-.896.248-1.783.52-2.658.814m-15.482 0A50.697 50.697 0 0112 13.489a50.702 50.702 0 017.74-3.342M6.75 15a.75.75 0 100-1.5.75.75 0 000 1.5zm0 0v-3.675A55.378 55.378 0 0112 8.443m-7.007 11.55A5.981 5.981 0 006.75 15.75v-1.5"/></svg>',
    ],
    [
        'slug'  => 'autism-spectrum',
        'title' => __( 'Autism Spectrum', 'mybrand' ),
        'desc'  => __( 'Structured, consistent care environments for autistic adults, developed using evidence-based approaches and sensory-aware design.', 'mybrand' ),
        'icon'  => '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.75" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z"/></svg>',
    ],
    [
        'slug'  => 'mental-health',
        'title' => __( 'Mental Health', 'mybrand' ),
        'desc'  => __( 'Recovery-focused support for people with mental health conditions, working closely with community mental health teams and families.', 'mybrand' ),
        'icon'  => '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.75" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09zM18.259 8.715L18 9.75l-.259-1.035a3.375 3.375 0 00-2.455-2.456L14.25 6l1.036-.259a3.375 3.375 0 002.455-2.456L18 2.25l.259 1.035a3.375 3.375 0 002.456 2.456L21.75 6l-1.035.259a3.375 3.375 0 00-2.456 2.456zM16.894 20.567L16.5 21.75l-.394-1.183a2.25 2.25 0 00-1.423-1.423L13.5 18.75l1.183-.394a2.25 2.25 0 001.423-1.423l.394-1.183.394 1.183a2.25 2.25 0 001.423 1.423l1.183.394-1.183.394a2.25 2.25 0 00-1.423 1.423z"/></svg>',
    ],
    [
        'slug'  => 'physical-disabilities',
        'title' => __( 'Physical Disabilities', 'mybrand' ),
        'desc'  => __( 'Personal care, mobility support, and assisted living services helping people with physical disabilities maintain independence.', 'mybrand' ),
        'icon'  => '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.75" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z"/></svg>',
    ],
    [
        'slug'  => 'complex-care',
        'title' => __( 'Complex &amp; Challenging Needs', 'mybrand' ),
        'desc'  => __( 'Highly specialist support for individuals with complex, co-occurring conditions including challenging behaviours and dual diagnoses.', 'mybrand' ),
        'icon'  => '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.75" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M11.42 15.17L17.25 21A2.652 2.652 0 0021 17.25l-5.877-5.877M11.42 15.17l2.496-3.03c.317-.384.74-.626 1.208-.766M11.42 15.17l-4.655 5.653a2.548 2.548 0 11-3.586-3.586l6.837-5.63m5.108-.233c.55-.164 1.163-.188 1.743-.14a4.5 4.5 0 004.486-6.336l-3.276 3.277a3.004 3.004 0 01-2.25-2.25l3.276-3.276a4.5 4.5 0 00-6.336 4.486c.091 1.076-.071 2.264-.904 2.95l-.102.085m-1.745 1.437L5.909 7.5H4.5L2.25 3.75l1.5-1.5L7.5 4.5v1.409l4.26 4.26m-1.745 1.437l1.745-1.437m6.615 8.206L15.75 15.75M4.867 19.125h.008v.008h-.008v-.008z"/></svg>',
    ],
];
?>
<section id="services" class="section section-services">
    <div class="container">
        <div class="section-header text-center">
            <span class="section-label"><?php esc_html_e( 'What We Do', 'mybrand' ); ?></span>
            <h2 class="section-title"><?php esc_html_e( 'Our Care Services', 'mybrand' ); ?></h2>
            <p class="section-desc"><?php esc_html_e( 'We deliver specialist, person-centred care across a wide range of needs — always with dignity, respect, and a commitment to independence at the heart of everything we do.', 'mybrand' ); ?></p>
        </div>

        <div class="services-grid">
            <?php foreach ( $services as $service ) : ?>
                <div class="service-card">
                    <div class="service-icon-wrap" aria-hidden="true">
                        <?php echo $service['icon']; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- trusted SVG markup ?>
                    </div>
                    <h3 class="service-title"><?php echo wp_kses_post( $service['title'] ); ?></h3>
                    <p class="service-desc"><?php echo esc_html( $service['desc'] ); ?></p>
                    <a class="service-link" href="<?php echo esc_url( home_url( '/services/' . $service['slug'] . '/' ) ); ?>">
                        <?php esc_html_e( 'Learn more', 'mybrand' ); ?>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3"/></svg>
                    </a>
                </div>
            <?php endforeach; ?>
        </div>

        <div class="section-cta text-center">
            <a class="btn btn-outline-blue" href="<?php echo esc_url( home_url( '/services/' ) ); ?>">
                <?php esc_html_e( 'View All Services', 'mybrand' ); ?>
            </a>
        </div>
    </div>
</section>
